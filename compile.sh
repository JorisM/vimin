...
./configure --prefix=/usr/local --with-features=huge --with-compiledby='Joris Morger' --enable-gui=auto --enable-luainterp --enable-rubyinterp --enable-pythoninterp --with-x --with-python-config-dir=/usr/lib/python2.7/config
make
sudo make install
